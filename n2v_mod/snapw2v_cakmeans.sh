...
#We're also feeding in the K-means output partition (rather than the modularity based one)
$WORD2VEC -train $GRAPH_RW.kmeans -output $OUTFILE.kmeans -size $SIZE -window $WINDOW -sample 1e-4 -negative $NS -hs 0 -cbow 0 -iter $ITERS -min-count 1 -classes $K -debug 2 -ct $CLUST_FILE.kmeans -ct-match-wt $BETA -pt $OUTFILE.bin -ct $CLUST_FILE.$BETA.gecmi -ct-match-wt $BETA 
